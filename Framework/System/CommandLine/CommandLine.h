...
constexpr uint8_t _KEY_BACKSPACE =	'\b'    ; // [backspace]
...
public:
	...
	void termDisplayClear	()  	{ tx_printf("\033[2J");		}; // terminal clear all
	void termResetCursor	()   	{ tx_printf("\033[H");		}; // cursor reset
	void termHideCursor		()    	{ tx_printf("\033[?25l");	}; // cursor invisible
	void termShowCursor		()    	{ tx_printf("\033[?25h");	}; // cursor visible
	void termHighLight		()     	{ tx_printf("\033[7m");		}; // reverse display
	void termUnHighLight	()  	{ tx_printf("\033[27m");	}; // normal display
	void termPos1			()	 	{ tx_printf("\r\033[1A");	}; // cursor to pos1
	void termDelete		(uint8_t n)	{ tx_printf("\033[%dP", n);	}; // Del n chars
	void termInsert		(uint8_t n)	{ tx_printf("\033[%d@", n);	}; // Ins n chars
	void termMoveUp		(uint8_t n) { tx_printf("\033[%dA", n);	}; // cursor move up
	void termMoveDown	(uint8_t n) { tx_printf("\033[%dB", n); }; // cursor move down
	void termMoveLeft	(uint8_t n) { tx_printf("\033[%dD", n); }; // cursor move left
	void termMoveRight	(uint8_t n) { tx_printf("\033[%dC", n);	}; // cursor move right
	void termMoveToCol 	(uint8_t n) { tx_printf("\0338%dG", n); 	}; // Move cursor to column n
	void termInsLines 	(uint8_t n) { tx_printf("\0338%dL", n); 	}; // Insert n blank lines
	void termDelLines 	(uint8_t n) { tx_printf("\0338%dM", n); 	}; // Delete n lines
	void termEraseChars (uint8_t n) { tx_printf("\033[%dX", n); 	}; // Erase n characters
	void termMoveUpRows	(uint8_t n) { tx_printf("\033[%dF", n); 	}; // Move cursor up n rows, to col 1
	void termMoveDownRows (uint8_t n) { tx_printf("\033[%dE", n);}; // Move cursor down n rows, to col 1
	void termMoveTo(uint8_t x, uint8_t y)  {tx_printf("\033[%d;%dH", x, y);	};  // cursor move to
	//	0 erase from cursor to end of display,
	//  1 erase from start of display to cursor,
	 // 2 erase display
	void termEraseDisplay (uint8_t n) { tx_printf("\033[%dJ", n); };
...
protected:
	...
	void incCmdPos(void) { if(_cmdPos < COMMAND_BUFFER_LEN) _cmdPos++;	};
	void decCmdPos(void) { if(_cmdPos > 0) _cmdPos--;	};
	void resCmdPos(void) { _cmdPos = 0; };
	void moveCmdRight(uint8_t startPos);
	void moveCmdLeft(uint8_t startPos);
	bool readNextChar(uint8_t &chr);

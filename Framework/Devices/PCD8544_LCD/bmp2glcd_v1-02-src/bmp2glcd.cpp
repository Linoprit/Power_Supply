...
void printHelp(void){
  cout << "bmp2glcd - Bitmap to GLCD bitmap ver 1.01 by S.Varjo 2008" <<endl
       ...
       << "\t-v\tverbose mode" << endl
       << "\t-pgm\tcrate header with __attribute__ ((progmem)) for AVR"<< endl
       << "\t-pic30\tcrate header whith __attribute__((space(auto_psv))) for PIC30" << endl<<endl
...
bool saveHeaderFile(string name, BMP &image){
  ...
  out << "//---------------------------------------------------------------------------"
      ...
      << "#define "<<basename.c_str()<<"_HEIGHT "<<image.TellHeight() <<" "<< endl

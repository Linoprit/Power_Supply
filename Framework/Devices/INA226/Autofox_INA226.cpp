...
//----------------------------------------------------------------------------
status AutoFox_INA226::Init(
		uint8_t aI2C_Address, double aShuntResistor_Ohms, double aMaxCurrent_Amps)
{
	mInitialized = false;

	mI2C_Address = (aI2C_Address << 1); // on STM32, 7-bit address must be left aligned

	//Check if there's a device (any I2C device) at the specified address.
	CALL_FN( CheckI2cAddress(/*mI2C_Address*/) );
...
status AutoFox_INA226::CheckI2cAddress(/*uint8_t aI2C_Address*/)
{
	HAL_StatusTypeDef result =
			HAL_I2C_IsDeviceReady(I2C_HANDLE, mI2C_Address, 10u, I2C_TIMEOUT);
...
{
	HAL_I2C_Master_Transmit(I2C_HANDLE, mI2C_Address, &aRegister, 1u, I2C_TIMEOUT);

	uint16_t buffer = 0;
	HAL_StatusTypeDef result =
			HAL_I2C_Master_Receive(I2C_HANDLE, mI2C_Address,
					reinterpret_cast<uint8_t*>(&buffer), 2u, I2C_TIMEOUT);

	aValue = HelpersLib::swapBytes(buffer);
...
status AutoFox_INA226::WriteRegister(uint8_t aRegister, uint16_t aValue)
{
	...
	buffer[0] = aRegister;
	buffer[1] = HelpersLib::hibyte(aValue);
	buffer[2] = HelpersLib::lobyte(aValue);

	HAL_StatusTypeDef result =
			HAL_I2C_Master_Transmit(I2C_HANDLE, mI2C_Address,
					reinterpret_cast<uint8_t*>(&buffer), 3u, I2C_TIMEOUT);
